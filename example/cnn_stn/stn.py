...
from tensorflow.keras.layers import Reshape, Dense, Layer
...
# mapping the output coordinates back to the input coordinates
def affine_grid_generator(B, H, W, theta):
	...
	# cast the affine parameters and sampling grid to float32 (required for matmul)
	...
	# reshape the sampling grid to (B, 2, H, W) (2 => X / Y)
...
class STN(Layer):
	...
	def build(self, input_shape):
		...
		self.localizationNet = Sequential([
			Conv2D(filters=self.filter // 4, kernel_size=3, input_shape=(self.H, self.W, self.C), 
				   activation="relu", kernel_initializer="he_normal"),
			MaxPool2D(),
			Conv2D(filters=self.filter // 2, kernel_size=3, activation="relu", kernel_initializer="he_normal"),
			MaxPool2D(),
			Conv2D(filters=self.filter, kernel_size=3, activation="relu", kernel_initializer="he_normal"),
			MaxPool2D(),
			GlobalAveragePooling2D()
		])

		# define the regressor network (output transformation parameters theta)
		self.regressorNet = tf.keras.Sequential([
			Dense(units = self.filter, activation="relu", kernel_initializer="he_normal"),
			Dense(units = self.filter // 2, activation="relu", kernel_initializer="he_normal"),
			Dense(units = 3 * 2, kernel_initializer="zeros", bias_initializer=self.output_bias),
